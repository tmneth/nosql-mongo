...
def remove_hospital(hospital_id):
    ...
    if result.deleted_count:
        return jsonify({"message": "Hospital removed successfully"}), 200
    else:
...
def add_department_to_hospital(hospital_id):
    ...
    try:
        ...
        return jsonify(
            {"message": "Department added to hospital successfully", "department_id": str(department_id)}), 201
    except KeyError:
        abort(400, description="Invalid data")
...
def remove_department_from_hospital(hospital_id, department_id):
    ...
    if result.modified_count:
        return jsonify({"message": "Department removed from hospital successfully"}), 200
    else:
...
def add_equipment_to_department(hospital_id, department_id):
    ...
    try:
        ...
        if result.modified_count:
            return jsonify({"message": "Equipment added to department successfully"}), 201
        else:
            abort(404, description="Department not found in the specified hospital")
    except KeyError:
        abort(400, description="Invalid data")
...
def add_facility_to_department(hospital_id, department_id):
    ...
    try:
        ...
        if result.modified_count:
            return jsonify({"message": "Facility added to department successfully"}), 201
        else:
            abort(404, description="Department not found in the specified hospital")
    except KeyError:
        abort(400, description="Invalid data")
...
def get_all_equipment_for_hospital(hospital_id):
    hospital = db.hospitals.find_one({"_id": ObjectId(hospital_id)}, {"departments.equipment": 1})
    ...
    for department in hospital.get("departments", []):
        all_equipment.extend(department.get("equipment", []))
...
def get_all_facilities_for_hospital(hospital_id):
    hospital = db.hospitals.find_one({"_id": ObjectId(hospital_id)}, {"departments.facilities": 1})
    print(hospital)
    if not hospital:
        abort(404, description="Hospital not found")
    ...
    for department in hospital.get("departments", []):
        all_facilities.extend(department.get("facilities", []))
